...
export async function getDashboardStats(userId) {
  ...
  const [user, expenses, bills, paidBills, goals, vault] = await Promise.all([
    ...
    prisma.transaction.count({
      where: { userId, type: 'BILL', isPaid: true },
    }),
  ...
  if (budget > 0) {
    ...
    const billsPaidPct = bills.length > 0 ? (paidBills / bills.length) * 100 : 100;
  ...
  return {
    ...
    bills: {
      total: bills.length,
      paid: paidBills,
      pending: bills.length - paidBills,
